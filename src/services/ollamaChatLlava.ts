...
import { apiGenLLava } from "./lvaModel";
...
export const OllamaChatLLava = async (inputMsg: any, conversationHistory: Message[]) => {
  ...
  const inputUser = {
    image: inputMsg.question.img,
    question: inputMsg.question.txt,
  };

  try {
    const comeResponse = await apiGenLLava(inputUser);
